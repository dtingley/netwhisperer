...
class Network:
    ...
    def __init__(self, window_size, window_middle, n_hidden_neurons):
        ...
        self._init_layers()
        self._pybrain_network = buildNetwork(self.n_input_neurons, self.n_hidden_neurons, self.n_output_neurons)
    ...
    def _init_layers(self):
        ...
        for (phoneme, traits) in corpus.phoneme_traits.iteritems():
            layer = zeros(len(self.phoneme_trait_neuron_names))
            for trait in traits:
    ...
    def wordSamples(self, word):
        ...
        for (letters_window, current_phoneme) in izip(windowIter(word.letters), word.phonemes):
            yield self._letters_to_layer(letters_window), phoneme_to_layer(current_phoneme)

    def train(self, training_set):
        _pybrain_network.train
